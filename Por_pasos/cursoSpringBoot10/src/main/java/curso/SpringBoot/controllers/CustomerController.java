    ...
    public ResponseEntity<?> putCliente(@RequestBody Customer customerput){
        ...
        for (Customer c: customers){
           ...
           if(c.getId() == customerput.getId()){
               ...
               return ResponseEntity.ok("Cliente modificado exitosamente: " + customerput.getId());

           }

        }

        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No se encontro el cliente con el id: " + customerput.getId());
    ...
    public ResponseEntity<?> delCliente(@PathVariable int id){
        ...
        for (Customer c: customers) {
            ...
            if(c.getId() == id){
                ...
                return ResponseEntity.status(HttpStatus.NO_CONTENT).body("Cliente eliminado exitosamente: " + id);

            }

        }

        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No se encontro el cliente con el id: " + id);
